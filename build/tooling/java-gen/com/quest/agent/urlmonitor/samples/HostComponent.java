 ...
 * the HostComponent topology object.
 * <p/>
    ...
    extends HostBaseObject
{
    ...
    public final static String NODE_NAME = "HostComponent";
    ...
    public HostComponent(Host host) {
        if (host == null) {
        ...
        throws TopologyException
    {
        ...
        if (!(_host == null)) {
            final TopologyNode _node95 = currentNode.createNode("host");
            if (__isIdentity("host")) {
                _node95 .setIsIdentity(true);
            }
            if (submitted.contains(_host.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node95 .setRefId(_host.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node95 .setTypeHint(_host.__getNodeName());
                _node95 .setId(_host.__getNodeId().toString());
                _host.addToNodeTree(_node95, unitService, submitted);
            }
        }
    }
    ...
    protected java.lang.Object __cloneIdOnly() {
        return new HostComponent(getHost());
    }
