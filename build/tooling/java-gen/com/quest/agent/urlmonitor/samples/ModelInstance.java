 ...
 * the ModelInstance topology object.
 * <p/>
    ...
    extends TopologyObject
{
    ...
    public final static String NODE_NAME = "ModelInstance";
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value202 = currentNode.createValue("name");
            _value202 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value202 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node203 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node203 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node203 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node203 .setTypeHint(_monitoringAgent.__getNodeName());
                _node203 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node203, unitService, submitted);
            }
        }
    }
