    ...
    extends TopologyObject
{
    ...
    public final static String NODE_NAME = "Object";
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value222 = currentNode.createValue("name");
            _value222 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value222 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node223 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node223 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node223 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node223 .setTypeHint(_monitoringAgent.__getNodeName());
                _node223 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node223, unitService, submitted);
            }
        }
    }
