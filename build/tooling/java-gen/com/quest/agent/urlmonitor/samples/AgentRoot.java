    ...
    extends ModelRoot
{
    ...
    public final static String NODE_NAME = "AgentRoot";
    ...
    private List<MonitoredURL> _instances;
    private boolean _replace_instances = false;

    public AgentRoot(String name) {
        if (name == null) {
        ...
        throws TopologyException
    {
        ...
        if (!(_name == null)) {
            final TopologyValue _value8 = currentNode.createValue("name");
            _value8 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value8 .setIsIdentity(true);
            }
        }
        if (!(_instances == null)) {
            for (TopologyObject list_element: _instances) {
                final TopologyNode _node9 = currentNode.createNode("instances");
                if (__isIdentity("instances")) {
                    _node9 .setIsIdentity(true);
                }
                if (_replace_instances) {
                    _node9 .setReplace(true);
                }
                _replace_instances = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node9 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node9 .setTypeHint(list_element.__getNodeName());
                    _node9 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node9, unitService, submitted);
                }
            }
        }
    }
     ...
     * 
     */
    public List<MonitoredURL> getInstances() {
        if (_instances == null) {
            _instances = new ArrayList<MonitoredURL>(3);
        }
        return _instances;
    }
     ...
     * 
     */
    public void setReplaceForInstances(boolean _replace) {
        _replace_instances = _replace;
    }
    ...
    public String toString() {
        ...
        builder.append("[");
        builder.append("AgentRoot");
        builder.append("@");
        builder.append(String.valueOf(hashCode()));
        builder.append(" extends ");
        builder.append("ModelRoot");
        builder.append(", ");
        builder.append("name");
        builder.append("=");
