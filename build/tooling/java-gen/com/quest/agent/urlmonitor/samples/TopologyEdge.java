 ...
 * the TopologyEdge topology object.
 * <p/>
    ...
    extends TopologyObject
{
    ...
    public final static String NODE_NAME = "TopologyEdge";
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value219 = currentNode.createValue("name");
            _value219 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value219 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node220 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node220 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node220 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node220 .setTypeHint(_monitoringAgent.__getNodeName());
                _node220 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node220, unitService, submitted);
            }
        }
    }
